...
static inline __attribute__((unused)) __u32 get_outer_src_ip(struct __sk_buff *skb) {
    __u8 ip_data[4];
...
static inline __attribute__((unused)) __u32 get_outer_dst_ip(struct __sk_buff *skb) {
    __u8 ip_data[4];
...
static inline __attribute__((unused)) __u16 ip_checksum(__u8 *data, int len) {
    __u32 sum = 0;
...
int gtp_decap_handler(struct __sk_buff* skb) {
    ...
    if (bpf_skb_load_bytes(skb, 0, pkt_data, sizeof(pkt_data)) < 0) {
        ...
        bpf_printk("[GTP] Failed to load first 64 bytes\n");
        return TC_ACT_OK;  // Phase-1 Fix (Finding 2): pass short frames, don't drop
    }

    bpf_printk("[ETH] Src bytes: %u %u %u\n", pkt_data[6], pkt_data[7], pkt_data[8]);
    bpf_printk("[ETH] Src bytes cont: %u %u %u\n", pkt_data[9], pkt_data[10], pkt_data[11]);
    bpf_printk("[ETH] Dst bytes: %u %u %u\n", pkt_data[0], pkt_data[1], pkt_data[2]);
    bpf_printk("[ETH] Dst bytes cont: %u %u %u\n", pkt_data[3], pkt_data[4], pkt_data[5]);

    __u16 eth_type = (__u16)pkt_data[12] << 8 | (__u16)pkt_data[13];
    bpf_printk("[ETH] Ethertype: 0x%x\n", eth_type);

    if (eth_type != ETH_P_IP) {
        /* Phase-1 Fix (Finding 3): non-IPv4 passthrough is not a drop */
        bpf_printk("[GTP] Non-IPv4 outer pkt, skipping\n");
        return TC_ACT_OK;  
    }
    update_stats(22, 1); 
    bpf_printk("[Parsing IP Header...]\n");

    if (ETH_HLEN + sizeof(struct iphdr) > sizeof(pkt_data)) {
        bpf_printk("[GTP] Not enough data for IPv4 header\n");
        return TC_ACT_SHOT;
    }
    ...
    bpf_printk("[IP] Ver:%u IHL:%u Proto:%u\n", ip_version, ihl, ip_protocol);
    bpf_printk("[IP] Src part1: %u.%u", pkt_data[14], pkt_data[15]);
    bpf_printk(".%u.%u\n", pkt_data[16], pkt_data[17]);
    bpf_printk("[IP] Dst part1: %u.%u", pkt_data[18], pkt_data[19]);
    ...
    if (udp_dest_port != 2152) {
        bpf_printk("[GTP] Not GTP-U (udp dst != 2152)\n");
        return TC_ACT_OK;  
    }
    update_stats(24, 1); 
    bpf_printk("[GTP] GTP-U detected, proceeding to decap\n");

    __u32 gtp_offset = ETH_HLEN + ip_hlen + sizeof(struct udphdr);
    if (gtp_offset > 78) {
        bpf_printk("[GTP] GTP offset too large\n");
        return TC_ACT_SHOT;
    }

    __u8 gtp_data[8];
    if (bpf_skb_load_bytes(skb, gtp_offset, gtp_data, sizeof(gtp_data)) < 0) {
        bpf_printk("[GTP] Unable to load GTP header\n");
        return TC_ACT_SHOT;
    }
...
__u8 gtp_version = (gtp_flags & GTP_FLAG_VERSION_MASK) >> 5;
    ...
    if (gtp_flags & (GTP_FLAG_E | GTP_FLAG_S | GTP_FLAG_PN)) {
        ...
        if (gtp_offset + gtp_hdr_len > skb->len) {
            bpf_printk("[GTP] GTP optional fields exceed skb len\n");
            return TC_ACT_SHOT;
        }
    }

    __u8 wire_qfi = 0;  /* QFI from PDU Session Container, if present */
    if (gtp_flags & GTP_FLAG_E) {
        ...
        bpf_printk("[GTP] Extension flag set, parsing headers at offset %u\n", ext_offset);
        
        if (bpf_skb_load_bytes(skb, ext_offset, &next_ext_type, 1) < 0) {
            bpf_printk("[GTP] Failed to read next ext header type\n");
            return TC_ACT_SHOT;
        }
        
        bpf_printk("[GTP] First ext header type: 0x%x at offset %u\n", next_ext_type, ext_offset);
        
        // Bounded loop (max 5) — kernel >= 5.3 verifies bounded loops; no unroll needed.
        for (int i = 0; i < 5 && next_ext_type != 0; i++) {  // Max 5 extensions to avoid verifier issues
            ...
            if (bpf_skb_load_bytes(skb, ext_offset, ext_hdr, sizeof(ext_hdr)) < 0) {
                bpf_printk("[GTP] Failed to load ext header %d\n", i);
                return TC_ACT_SHOT;
            }
            ...
            if (next_ext_type == 0x85) {  // PDU Session Container
                /* QFI is octet 3 of the container: ext_hdr[0]=Length, [1]=PDU-type,
                 * [2]=QFI, [3]=NextExtType (3GPP TS 38.415 / OVS gtpu_ext_hdr_pdu_sc). */
                wire_qfi = ext_hdr[2] & 0x3F;
                bpf_printk("[5G-QoS] parsed QFI: %u\n", wire_qfi);
            }
            
            __u32 ext_hdr_size = ext_len * 4;
            if (ext_hdr_size < 4) {
                bpf_printk("[GTP] Invalid ext header length: %u\n", ext_len);
                return TC_ACT_SHOT;
            }
            
            bpf_printk("[GTP] Ext %d: size=%u, hdr_len=%u\n", 
            ...
            if (bpf_skb_load_bytes(skb, ext_offset, &next_ext_type, 1) < 0) {
                bpf_printk("[GTP] Failed to read next ext type\n");
                break;
            }
            
            bpf_printk("[GTP] Ext header %d: len=%u, next=0x%x\n", i, ext_len, next_ext_type);
        }
    }
    
    bpf_printk("[GTP] Total GTP header len: %u bytes\n", gtp_hdr_len);

    __u32 outer_hdr_len = ip_hlen + sizeof(struct udphdr) + gtp_hdr_len;
    __u32 inner_ip_offset = ETH_HLEN + outer_hdr_len;
    
    bpf_printk("[GTP] Offsets: outer_hdr=%u, inner_ip=%u\n", outer_hdr_len, inner_ip_offset);
    
    if (inner_ip_offset > skb->len) {
        bpf_printk("[GTP] inner offset %u beyond skb len %u\n", inner_ip_offset, skb->len);
        return TC_ACT_SHOT;
    }

    /* Phase-1 Fix (Finding 1): removed bpf_skb_set_tunnel_key/opt block.
     * Tunnel metadata was never consumed (decap strips headers + redirects) and its
     * unconditional TC_ACT_SHOT-on-failure dropped all uplink on kernels lacking
     * ip_tunnel/LWTUNNEL. */
    ...
    bpf_printk("[GTP] Inner IP: ver=%u, IHL=%u, byte=%x\n", 
                     inner_version, inner_ihl, inner_ip_data[0]);
    
    if (inner_version != 4) {
        bpf_printk("[GTP] ERROR: Inner packet not IPv4 (ver=%u)\n", inner_version);
        bpf_printk("[GTP] Invalid inner packet at offset %u\n", inner_ip_offset);
        return TC_ACT_SHOT;
    }
    ...
    bpf_printk("[GTP] Inner src IP (host order): 0x%x\n", inner_src_ip);

    struct ue_session_key session_key = {.ue_ip = inner_src_ip};
    bpf_printk("[GTP] Looking up session for IP: 0x%x\n", session_key.ue_ip);
    ...
    if (session_info == NULL) {
        ...
        update_stats(STATS_PKT_DROPPED, 1);
        bpf_printk("[GTP] Session NOT FOUND for UE IP 0x%x\n", inner_src_ip);
        return TC_ACT_SHOT;
    }
...
bpf_printk("[GTP] Session FOUND! TEID exp: %u",
    ...
    if (!(session_info->session_flags & 1)) {
        ...
        update_stats(STATS_PKT_DROPPED, 1);
        bpf_printk("[GTP] Session inactive\n");
        return TC_ACT_SHOT;
    }
    ...
    if (session_info->teid_ul_in != 0 &&
        ...
        update_stats(STATS_PKT_DROPPED, 1);
        bpf_printk("[GTP] TEID mismatch: got %u exp %u\n",
                         gtp_teid, session_info->teid_ul_in);
        return TC_ACT_SHOT;
    }
    ...
    /* Stage 4 (Jordan req): hand the QFI to OVS via skb->mark. Prefer the QFI
     * parsed off the wire (PDU Session Container); else use the session's QFI.
     * mark[5:0] = QFI; gtp_mark.c restores it after bpf_redirect to gtp_veth0. */
    __u8 qfi = wire_qfi ? wire_qfi : session_info->qfi;
    session_info->qfi = qfi;
    __u32 qfi_mark = (__u32)(qfi & 0x3F);
    session_info->metadata_mark = bpf_htonl(qfi_mark);
    skb->mark = qfi_mark;
    bpf_printk("[5G-QoS] decap QFI=%u -> skb->mark=0x%x\n", qfi, qfi_mark);

    // Guard against invalid head removal
    ...
    if (ret < 0) {
        bpf_printk("[GTP] bpf_skb_adjust_room failed: %d\n", ret);
        update_stats(STATS_ADJUST_HEAD_FAIL, 1);
        update_stats(STATS_UL_ERRORS, 1);
        return TC_ACT_SHOT;
    }
    ...
    if (bpf_skb_store_bytes(skb, 0, new_eth_hdr, ETH_HLEN, 0) < 0) {
        bpf_printk("[GTP] Failed to store new ethernet header\n");
        update_stats(STATS_UL_ERRORS, 1);
        return TC_ACT_SHOT;
    }
...
struct config_value *ovs_val =
    ...
    if (ovs_val && ovs_val->value > 0) {
        bpf_printk("[GTP] Redirecting to gtp_veth0 ifindex: %u (ingress)\n", ovs_val->value);
        return bpf_redirect(ovs_val->value, BPF_F_INGRESS);
    }

    if (session_info->ovs_ifindex > 0) {
        bpf_printk("[GTP] Redirecting to session ovs ifindex: %u (ingress)\n", session_info->ovs_ifindex);
        return bpf_redirect(session_info->ovs_ifindex, BPF_F_INGRESS);
    }

    bpf_printk("[GTP] ERROR: No valid OVS interface found, dropping packet\n");
    update_stats(STATS_UL_ERRORS, 1);
    return TC_ACT_SHOT;
}

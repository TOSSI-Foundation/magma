> ## TOSSI‑Foundation / magma
>
> This repository is maintained by the **TOSSI Foundation**, and this — `main` — is its
> authoritative branch. `main` is a snapshot of **Magma v1.9** (upstream
> [`magma/magma`](https://github.com/magma/magma), tag `v1.9`), imported as the project
> baseline with history squashed to a single import commit.
>
> From this baseline, TOSSI is the maintaining home for the codebase; eBPF user‑plane work and
> other changes live on dedicated branches (e.g. `ebpf`). Upstream Magma is BSD‑3‑Clause; all
> original copyrights and licenses are retained.
